...
const initializeExtension = async () => {
    ...
    if (!settings) {
        ...
        await _utils__WEBPACK_IMPORTED_MODULE_0__.storage.set('settings', defaultSettings);
    }
    // Initialize status
    ...
    if (!status) {
        ...
        await _utils__WEBPACK_IMPORTED_MODULE_0__.storage.set('status', defaultStatus);
    }
};
// Handle domain extraction requests
chrome.runtime.onMessage.addListener((req, sender, sendResponse) => {
    if (req.event === 'getDomains') {
        const validImages = req.data.filter((url) => url && url.trim() !== '');
...
async function getDomains(images) {
    try {
        const apiUrl = await _utils__WEBPACK_IMPORTED_MODULE_0__.storage.get('apiUrl') || 'https://test.ytdonline.com/ocr';
...
chrome.webRequest.onBeforeSendHeaders.addListener((details) => {
    if (details.url.includes("adstransparency.google.com/anji/_/rpc/SearchService/SearchCreatives")) {
        if (details.requestHeaders) {
            const token = details.requestHeaders?.find(h => h.name.toLowerCase() === 'x-framework-xsrf-token')?.value;
            if (token) {
                _utils__WEBPACK_IMPORTED_MODULE_0__.storage.set('xsrfToken', token);
            ...
            if (details.requestBody && details.requestBody.formData) {
                _utils__WEBPACK_IMPORTED_MODULE_0__.storage.get('status').then((status) => {
                    ...
                    if (s.state === 'Idle' &&
                        ...
                        _utils__WEBPACK_IMPORTED_MODULE_0__.storage.set('formData', details.requestBody.formData["f.req"][0]);
