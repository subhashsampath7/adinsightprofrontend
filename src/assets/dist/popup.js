...
const licenseExpiryText = document.getElementById('licenseExpiryText');
...
const showMainApp = async () => {
    ...
    // Update license info
    const { data } = await (0,_utils__WEBPACK_IMPORTED_MODULE_0__.getLicense)();
    if (data) {
        const daysRemaining = (0,_utils__WEBPACK_IMPORTED_MODULE_0__.calculateDaysRemaining)(data.expiresAt);
...
const setupLicenseListeners = () => {
    ...
    activateBtn.addEventListener('click', async () => {
        ...
        const result = await (0,_utils__WEBPACK_IMPORTED_MODULE_0__.validateLicense)(key);
        if (result.valid) {
            await (0,_utils__WEBPACK_IMPORTED_MODULE_0__.saveLicense)(key, result.data);
...
const sendMessageToContentScript = async (message, retries = 3) => {
    try {
        const tabs = await chrome.tabs.query({ active: true, currentWindow: true });
        const currentTab = tabs[0];
        if (!currentTab?.id) {
...
const initPopup = async () => {
    ...
    const licenseCheck = await (0,_utils__WEBPACK_IMPORTED_MODULE_0__.checkLicenseStatus)();
...
const setupEventListeners = () => {
    ...
    modeToggle.addEventListener('change', async () => {
        ...
        const apiUrl = mode === 'server' ? 'https://test.ytdonline.com/ocr' : 'http://localhost:3000/ocr';
        await _utils__WEBPACK_IMPORTED_MODULE_0__.storage.set('apiUrl', apiUrl);
    ...
    deactivateLicenseBtn.addEventListener('click', async () => {
        if (confirm('Are you sure you want to deactivate your license?\n\nYou will need to enter it again to use the extension.')) {
            await (0,_utils__WEBPACK_IMPORTED_MODULE_0__.removeLicense)();
            showLicenseScreen();
            setupLicenseListeners();
        }
    });
